...
public class Astar_Manager : MonoBehaviour
{
    ...
    public float volume(Vector3 v3)
    {
    ...
    private void Awake()
    {
        ...
        foreach (var transforms in Component.FindObjectsOfType<Transform>())
        {
            if (volume(transforms.localScale) < 1)
            {
                continue;
            }
            _pointGrid.AddObstaclesPointsToGrid(transforms.gameObject);
        }
    }
