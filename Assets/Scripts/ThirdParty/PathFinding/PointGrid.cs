...
public class PointGrid : MonoBehaviour
{
    ...
    public string UnwalkableTag = "GoAround";


    public List<PointNode> grid = new List<PointNode>();

    public Transform pointsmaller, pointbigger;
    ...
    {
        smallerBorder_X = pointsmaller.position.x;
        smallerBorder_Z = pointsmaller.position.z;

        biggerBorder_X = pointbigger.position.x;
        biggerBorder_Z = pointbigger.position.z;
    ...
    public bool CheckIfPointsLinkedTogether(PointNode point1,PointNode point2)
    {
        ...
        RaycastHit hit;
        if(Physics.Linecast(point1.position,point2.position, out hit))
        {
            if (hit.transform.gameObject.CompareTag(UnwalkableTag))
            {
                return false;
            }
        }
    ...
    bool CheckIsWalkabel(Vector3 pointNodePosition)
    {
        ...
        Collider[] c = Physics.OverlapSphere(pointNodePosition, 0.2f);
        foreach(Collider ca in c){
            if (ca.gameObject.CompareTag(UnwalkableTag))
            {
                return false;
            }
        }
